...
//
//  Created by Josh Everett on 13/02/2025.
//
...
struct MrzCaptureView: UIViewControllerRepresentable{
    ...
    let onMrzCapture : ((DocumentScanningResult<ParsedResult>) -> Void)
    func makeUIViewController(context: Context) -> MRZCaptureViewController {
        return MRZCaptureViewController(onMrzCapture: onMrzCapture)
    }
    
    func updateUIViewController(_ uiViewController: MRZCaptureViewController, context: Context) {
        print("Updated")
    }
    
}

class MRZCaptureViewController: UIViewController {
    // MARK: UI objects
    private let previewView = PreviewView()
    private let cutoutView = UIView()
    private let maskLayer = CAShapeLayer()
    private let onMrzCapture : ((DocumentScanningResult<ParsedResult>) -> Void)
    
    init(onMrzCapture: @escaping ((DocumentScanningResult<ParsedResult>) -> Void)){
        self.onMrzCapture = onMrzCapture
        super.init(nibName: nil, bundle: nil) // Then call super.init()
    }
...
extension MRZCaptureViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
    func captureOutput(
            ...
            scanner.scanFrame(
                ...
                completionHandler: { [weak self] result in
                    ...
                    case .success(let scanningResult):
                        self?.onMrzCapture(scanningResult)
